 ...
 *
 */
function heritage_body_classes($classes)
{
...
add_filter('body_class', 'heritage_body_classes');

/**
 * Flush out the transients used in heritage_categorized_blog.
 *
 * @return bool Whether transients were deleted.
 * @author Bop Design
 *
 */
function heritage_category_transient_flusher()
{
	...
	// Like, beat it. Dig?
	return delete_transient('heritage_categories');
}

add_action('delete_category', 'heritage_category_transient_flusher');
add_action('save_post', 'heritage_category_transient_flusher');
 ...
 *
 */
function heritage_content_more_link()
{
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__('Read More', 'heritage') . '...</a>';
}

add_filter('the_content_more_link', 'heritage_content_more_link');
 ...
 *
 */
function heritage_excerpt_more($more)
{
	return sprintf(' <a class="more-link" href="%1$s">%2$s</a>', get_permalink(get_the_ID()), esc_html__('Read more...', 'heritage'));
}

add_filter('excerpt_more', 'heritage_excerpt_more');
 ...
 *
 */
function heritage_get_the_content($content)
{
	return !empty($content) ? $content : false;
}

add_filter('the_content', 'heritage_get_the_content', 20);
 ...
 *
 */
function heritage_custom_mime_types($mimes)
{
...
add_filter('upload_mimes', 'heritage_custom_mime_types');
 ...
 * @author Bop Design
 */
function heritage_include_svg_icons()
{
	// Add the path to your SVG directory inside your theme.
	$svg_path = '/build/images/icons/';
...
add_action('wp_footer', 'heritage_include_svg_icons', 9999);
 ...
 *
 */
function heritage_display_customizer_header_scripts()
{
	// Check for header scripts.
	$scripts = get_theme_mod('heritage_header_scripts');
	...
	echo heritage_get_the_content($scripts);
}

add_action('wp_head', 'heritage_display_customizer_header_scripts', 999);
 ...
 *
 */
function heritage_display_customizer_footer_scripts()
{
	// Check for footer scripts.
	$scripts = get_theme_mod('heritage_footer_scripts');
...
add_action('wp_footer', 'heritage_display_customizer_footer_scripts', 999);
...
function heritage_add_og_tags()
{
	...
	$default_base_description = (get_bloginfo('description')) ? get_bloginfo('description') : esc_html__('Visit our website to learn more.', 'heritage');
	...
	if (is_category() || is_tag() || is_tax()) {
		...
		$specify        = (is_category()) ? esc_html__('categorized in', 'heritage') : esc_html__('tagged with', 'heritage');
		$queried_object = get_queried_object();
		$card_url       = get_term_link($queried_object);
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = sprintf(esc_html__('Posts %1$s %2$s.', 'heritage'), $specify, $term_name);
	...
	if (is_search()) {
		...
		$card_long_description = sprintf(esc_html__('Search results for %s.', 'heritage'), $search_term);
		$card_description      = $card_long_description;
	}
...
add_action('wp_head', 'heritage_add_og_tags');

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function heritage_pingback_header()
{
...
add_action('wp_head', 'heritage_pingback_header');
 ...
 *
 */
function heritage_remove_archive_title_prefix($block_title)
{
...
add_filter('get_the_archive_title', 'heritage_remove_archive_title_prefix');
 ...
 * @author Corey Collins
 */
function heritage_disable_wpautop_for_gutenberg()
{
...
add_filter('init', 'heritage_disable_wpautop_for_gutenberg', 9);
