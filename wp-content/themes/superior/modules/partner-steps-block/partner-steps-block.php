<?php
$version = rand(); 
wp_enqueue_style('partner-steps-block-styles', get_template_directory_uri() . '/static/css/modules/partner-steps-block/partner-steps-block.css', '', $version, 'all');
            ...
            <div class="steps"><?php
                    ...
                    <div class="step">
                        ...
                        <div class="content-col">
                            <?php if (!empty($heading)) { ?>
                                <h5 class="<?php echo esc_attr($blockSettings['heading_color']); ?>"><?php echo $index.". ".$heading; ?></h5>
                            <?php }
                            if (!empty($content)) { ?>
                                <div class="content"><?php echo $content; ?></div>
                            <?php }
                            $numrows = count( get_sub_field( 'buttons' ) ); 
                            if (have_rows('buttons')) {
                               ?>
                                <div class="btns <?php if($numrows > 2){ echo "multiplebtns"; } ?>">
                                    <?php while (have_rows('buttons')) {
                                        ...
                                        $buttonClass = ('fill' === $button_style) ? 'has-background btn-' . $button_color : 'is-outlined btn-outline-' . $button_color;

                                            // Print our block container with options.
                                        printf('<div class="wp-block-button"><a href="%s" class="custom-btn %s">%s</a></div>', esc_url($button['url']), esc_attr($buttonClass), esc_attr($button['title'])); 
                                    } ?>
                                </div>
                            <?php } ?>
                        </div>
