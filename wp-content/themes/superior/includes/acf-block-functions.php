...
//	$post_id = is_home() ? get_option('page_for_posts', true) : get_the_ID();
	...
	if ($args['background_type']) {
		if ('color' === $args['background_type'] && $background_options['background_color']['all_color_picker']) {
			$background_color = $background_options['background_color']['all_color_picker'];
		...
		if (('image' === $args['background_type'] || 'video' === $args['background_type'])) {
			if ($background_options['has_overlay']) {
				...
				$overlay_color = $background_options['overlay_color']['all_color_picker'];
...
function get_template_acf_block_settings($block)
{
	...
	if ($display_options['heading_color']['all_color_picker']) {
		$blockSettings['heading_color'] = ' has-heading-color text-' . esc_attr($display_options['heading_color']['all_color_picker']);
	}

	// Set the custom font color.
	if ($display_options['text_color']['all_color_picker']) {
		$blockSettings['text_color'] = ' has-text-color text-' . esc_attr($display_options['text_color']['all_color_picker']);
	}
...
function heritage_display_block_button($button)
{
	...
	$buttonClass = ('fill' === $button_style) ? 'has-background btn-heritage-gold btn-' . $button['button_color']['all_color_picker'] : 'is-outlined btn-outline-' . $button['button_color']['all_color_picker'];
...
{
	return [
		esc_html__('Heritage Blue', 'heritage') => '#236192',
		esc_html__('Dark Blue', 'heritage')     => '#002e5d',
		esc_html__('Blue', 'heritage')          => '#5e8ab4',
		esc_html__('Baby Blue', 'heritage')     => '#9bb8d3',
		esc_html__('Light Blue', 'heritage')    => '#d1dde6',
		esc_html__('Heritage Gold', 'heritage') => '#ed9c21',
		esc_html__('Yellow', 'heritage')        => '#fed141',
		esc_html__('Dark Metal', 'heritage')    => '#1d252d',
		esc_html__('Gray', 'heritage')          => '#888b8d',
		esc_html__('Light Gray', 'heritage')    => '#f2f2f2',
		esc_html__('White', 'heritage')         => '#fff',
		esc_html__('Black', 'heritage')         => '#000',
	];
}
